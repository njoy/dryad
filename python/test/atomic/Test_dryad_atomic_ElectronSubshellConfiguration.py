...
class Test_dryad_ElectronSubshellConfiguration( unittest.TestCase ) :
    ...
    def test_component( self ) :
        ...
        def verify_chunk_without_transitions( self, chunk ) :
            ...
            self.assertEqual( 'K', chunk.identifier )
        ...
        def verify_chunk_with_transitions( self, chunk ) :
            ...
            self.assertEqual( 'L2', chunk.radiative_transitions[0].originating_shell )
            self.assertEqual( 'L3', chunk.radiative_transitions[1].originating_shell )
            ...
            self.assertEqual( 'L1', chunk.non_radiative_transitions[0].originating_shell )
            self.assertEqual( 'L1', chunk.non_radiative_transitions[1].originating_shell )
            self.assertEqual( 'L1', chunk.non_radiative_transitions[2].originating_shell )
            self.assertEqual( 'L2', chunk.non_radiative_transitions[3].originating_shell )
            self.assertEqual( 'L2', chunk.non_radiative_transitions[4].originating_shell )
            self.assertEqual( 'L3', chunk.non_radiative_transitions[5].originating_shell )
            self.assertEqual( 'L1', chunk.non_radiative_transitions[0].emitting_shell )
            self.assertEqual( 'L2', chunk.non_radiative_transitions[1].emitting_shell )
            self.assertEqual( 'L3', chunk.non_radiative_transitions[2].emitting_shell )
            self.assertEqual( 'L2', chunk.non_radiative_transitions[3].emitting_shell )
            self.assertEqual( 'L3', chunk.non_radiative_transitions[4].emitting_shell )
            self.assertEqual( 'L3', chunk.non_radiative_transitions[5].emitting_shell )
        ...
        chunk = ElectronSubshellConfiguration( id = 'K', energy = 538, population = 2. )

        verify_chunk_without_transitions( self, chunk )

        # the data is given explicitly for a non- radiative transition without a transition energy
        chunk = ElectronSubshellConfiguration( id = 'K', energy = 538, population = 2.,
                                               radiative = [ RadiativeTransitionData( 'L2', 0.00190768, 523.09 ),
                                                             RadiativeTransitionData( 'L3', 0.00380027, 523.13 ) ],
                                               non_radiative = [ NonRadiativeTransitionData( 'L1', 'L1', 0.178644 , 478.82 ),
