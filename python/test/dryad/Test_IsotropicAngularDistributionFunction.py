...
class Test_IsotropicAngularDistributionFunction( unittest.TestCase ) :
    ...
    def test_component( self ) :
        ...
        def verify_normalised_chunk( self, chunk ) :

            # verify content
            self.assertAlmostEqual( 0.5, chunk.value )
        ...
        def verify_unnormalised_chunk( self, chunk ) :

            # verify content
            self.assertAlmostEqual( 1., chunk.value )

            self.assertAlmostEqual( -1., chunk.lower_cosine_limit )
            self.assertAlmostEqual(  1., chunk.upper_cosine_limit )

            # verify evaluation
            self.assertAlmostEqual( 1., chunk( cosine = -1.0 ) )
            self.assertAlmostEqual( 1., chunk( cosine = -0.5 ) )
            self.assertAlmostEqual( 1., chunk( cosine =  0.0 ) )
            self.assertAlmostEqual( 1., chunk( cosine =  0.5 ) )
            self.assertAlmostEqual( 1., chunk( cosine =  1.0 ) )
            ...
            # verify integration
            self.assertAlmostEqual( 2.0, chunk.integral )
            ...
            self.assertEqual( 0, legendre.order )
            self.assertEqual( 1, len( legendre.coefficients ) )
            self.assertAlmostEqual( 1., legendre.coefficients[0] )
            ...
            self.assertAlmostEqual( -1.0, tabulated.cosines[0] )
            self.assertAlmostEqual(  1.0, tabulated.cosines[1] )
            self.assertAlmostEqual(  1., tabulated.values[0] )
            self.assertAlmostEqual(  1., tabulated.values[1] )
            ...
            self.assertAlmostEqual( -1.0, linear.cosines[0] )
            self.assertAlmostEqual(  1.0, linear.cosines[1] )
            self.assertAlmostEqual(  1., linear.values[0] )
        ...
        # the data is constructed as a normalised distribution
        chunk = IsotropicAngularDistributionFunction()

        verify_normalised_chunk( self, chunk )

        # the data is constructed as an unnormalised distribution
        chunk = IsotropicAngularDistributionFunction( value = 1. )

        verify_unnormalised_chunk( self, chunk )
