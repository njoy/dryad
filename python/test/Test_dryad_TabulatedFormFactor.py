...
class Test_dryad_TabulatedFormFactor( unittest.TestCase ) :
    ...
    def test_component( self ) :
        ...
        def verify_chunk1( self, chunk ) :
            ...
            same = TabulatedFormFactor( [ 1., 4. ], [ 0., 3. ] )
            threshold = TabulatedFormFactor( [ 2., 4. ], [ 0., 2. ] )
            nonzerothreshold = TabulatedFormFactor( [ 2., 4. ], [ 1., 3. ] )
            small = TabulatedFormFactor( [ 1., 3. ], [ 0., 2. ] )
        ...
        def verify_chunk2( self, chunk ) :
            ...
            same = TabulatedFormFactor( [ 1., 4. ], [ 0., 3. ] )
            threshold = TabulatedFormFactor( [ 2., 4. ], [ 0., 2. ] )
            nonzerothreshold = TabulatedFormFactor( [ 3., 4. ], [ 1., 2. ] )
            small = TabulatedFormFactor( [ 1., 3. ], [ 0., 2. ] )
        ...
        def verify_chunk3( self, chunk ) :
            ...
            temp = TabulatedFormFactor( [ 1., 4. ], [ 4., 1. ] )
        ...
        chunk = TabulatedFormFactor( energies = [ 1., 2., 3., 4. ],
                                     values = [ 4., 3., 2., 1. ],
        ...
        chunk = TabulatedFormFactor( energies = [ 1., 2., 2., 3., 4. ],
                                     values = [ 4., 3., 4., 3., 2. ],
                                     interpolant = InterpolationType.LinearLinear )
        ...
        chunk = TabulatedFormFactor( energies = [ 1., 2., 3., 4., 4. ], # <-- jump at end
    ...
    def test_failures( self ) :
        ...
        with self.assertRaises( Exception ) :
            ...
            chunk = TabulatedFormFactor( energies = [], values = [] )

        with self.assertRaises( Exception ) :

            chunk = TabulatedFormFactor( energies = [ 1. ], values = [ 4. ] )
        ...
        with self.assertRaises( Exception ) :
            ...
            chunk = TabulatedFormFactor( energies = [ 1., 3., 2., 4. ],
                                         values = [ 4., 3., 2., 1. ] )

        # the x grid contains a triple x value
        with self.assertRaises( Exception ) :

            chunk = TabulatedFormFactor( energies = [ 1., 2., 2., 2., 3., 4. ],
                                         values = [ 4., 3., 3., 3., 2., 1. ] )

        # the x grid has a jump at the beginning
        with self.assertRaises( Exception ) :

            chunk = TabulatedFormFactor( energies = [ 1., 1., 3., 4. ],
                                         values = [ 4., 3., 1., 4. ] )

        # the x grid has a jump at the end
        with self.assertRaises( Exception ) :

            chunk = TabulatedFormFactor( energies = [ 1., 2., 4., 4. ],
                                         values = [ 4., 3., 1., 4. ] )
