...
SCENARIO( "createTabulatedComptonProfiles" ) {
  ...
  GIVEN( "instances of cdfs" ) {
    ...
    WHEN( "an instance of photoatomic ACE ComptonProfileBlock" ) {

      auto profiles = table.comptonProfileBlock().value();
      ...
      THEN( "Compton profiles can be derived" ) {
        ...
        CHECK( id::ElectronSubshellID( "1s1/2" ) == chunk[0].subshellIdentifier() );
        ...
        CHECK_THAT( 0.  , WithinRel( chunk[0].momentum().front() ) );
        CHECK_THAT( 100., WithinRel( chunk[0].momentum().back() ) );
        CHECK_THAT( 1.690581458877      , WithinRel( chunk[0].values().front() ) );
        CHECK_THAT( 5.17728126393395e-11, WithinRel( chunk[0].values().back() ) );
        CHECK( 30 == chunk[0].boundaries()[0] );
        CHECK( InterpolationType::LinearLinear == chunk[0].interpolants()[0] );

        auto pdf = chunk[0].pdf();
