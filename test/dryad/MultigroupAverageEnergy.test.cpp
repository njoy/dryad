...
SCENARIO( "MultigroupAverageEnergy" ) {
  ...
  GIVEN( "grouped data" ) {
    ...
    WHEN( "the data is given explicitly" ) {
      ...
      MultigroupAverageEnergy chunk( std::move( boundaries ), std::move( values ) );

      THEN( "a MultigroupAverageEnergy can be constructed and members can be tested" ) {
      ...
      THEN( "arithmetic operations can be performed" ) {
        ...
        MultigroupAverageEnergy result( { 0., 5. }, { 0. } );
        MultigroupAverageEnergy other( { 1., 2., 3., 4., 5. }, { 1., 2., 3., 4. } );
        MultigroupAverageEnergy different( { 1., 2., 3., 4. }, { 1., 2., 3. } );
  ...
  GIVEN( "invalid data for an MultigroupAverageEnergy object" ) {
    ...
    WHEN( "there are not enough values in the boundaries or values" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( MultigroupAverageEnergy( one, empty ) );
      } // THEN
    } // WHEN
    ...
    WHEN( "the boundaries or values do not have the current number of values" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( MultigroupAverageEnergy( std::move( boundaries ), std::move( values ) ) );
      } // THEN
    } // WHEN
