...
void verifyCorrectUnionisation( const ProjectileTarget&, bool );
...
SCENARIO( "ProjectileTarget" ) {
  ...
  GIVEN( "unioniseCrossSections" ) {

    WHEN( "an instance of ProjectileTarget is given" ) {

      ProjectileTarget chunk = makeProjectiletarget( false );

      THEN( "the cross section grids can be unionised including summation cross sections" ) {

        chunk.unioniseCrossSections( false );

        verifyCorrectUnionisation( chunk, false );
      } // THEN

      chunk = makeProjectiletarget( false );

      THEN( "the cross section grids can be unionised excluding summation cross sections" ) {

        chunk.unioniseCrossSections( true );

        verifyCorrectUnionisation( chunk, true );
      } // THEN
    } // WHEN
  } // GIVEN
...
ProjectileTarget makeProjectiletarget( bool normalise ) {
  ...
  return ProjectileTarget( std::move( projectile ), std::move( target ),
                           ...
                           std::nullopt, std::nullopt, normalise );
}
