...
SCENARIO( "MultigroupCrossSection" ) {
  ...
  GIVEN( "grouped data" ) {
    ...
    WHEN( "the data is given explicitly" ) {
      ...
      MultigroupCrossSection chunk( std::move( boundaries ), std::move( values ) );

      THEN( "a MultigroupCrossSection can be constructed and members can be tested" ) {
      ...
      THEN( "arithmetic operations can be performed" ) {
        ...
        MultigroupCrossSection result( { 0., 5. }, { 0. } );
        MultigroupCrossSection other( { 1., 2., 3., 4., 5. }, { 1., 2., 3., 4. } );
        MultigroupCrossSection different( { 1., 2., 3., 4. }, { 1., 2., 3. } );
  ...
  GIVEN( "invalid data for an MultigroupCrossSection object" ) {
    ...
    WHEN( "there are not enough values in the boundaries or values" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( MultigroupCrossSection( one, empty ) );
      } // THEN
    } // WHEN
    ...
    WHEN( "the boundaries or values do not have the current number of values" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( MultigroupCrossSection( std::move( boundaries ), std::move( values ) ) );
      } // THEN
    } // WHEN
