...
SCENARIO( "MultigroupMultiplicity" ) {
  ...
  GIVEN( "grouped data" ) {
    ...
    WHEN( "the data is given explicitly" ) {
      ...
      MultigroupMultiplicity chunk( std::move( boundaries ), std::move( values ) );

      THEN( "a MultigroupMultiplicity can be constructed and members can be tested" ) {
      ...
      THEN( "arithmetic operations can be performed" ) {
        ...
        MultigroupMultiplicity result( { 0., 5. }, { 0. } );
        MultigroupMultiplicity other( { 1., 2., 3., 4., 5. }, { 1., 2., 3., 4. } );
        MultigroupMultiplicity different( { 1., 2., 3., 4. }, { 1., 2., 3. } );
  ...
  GIVEN( "invalid data for an MultigroupMultiplicity object" ) {
    ...
    WHEN( "there are not enough values in the boundaries or values" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( MultigroupMultiplicity( one, empty ) );
      } // THEN
    } // WHEN
    ...
    WHEN( "the boundaries or values do not have the current number of values" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( MultigroupMultiplicity( std::move( boundaries ), std::move( values ) ) );
      } // THEN
    } // WHEN
