...
SCENARIO( "createLegendreAngularDistribution" ) {
  ...
  GIVEN( "ENDF MF4 MixedDistributions" ) {

    using MixedDistributions = njoy::ENDFtk::section::Type< 4 >::MixedDistributions;

    using Tape = njoy::ENDFtk::tree::Tape;
    auto tape = njoy::ENDFtk::tree::fromFile< Tape >( "n-010_Ne_022.endf" );
    auto section = tape.materials().front().section( 4, 2 ).parse< 4 >();
    auto distribution = std::get< MixedDistributions >( section.distributions() );

    WHEN( "a single parsed MF4 MixedDistributions is given" ) {

      THEN( "it can be converted" ) {

        auto chunk1 = endf::read::createMixedAngularDistributions( distribution, false );
        auto chunk2 = endf::read::createMixedAngularDistributions( distribution, true );
