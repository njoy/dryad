...
SCENARIO( "insertScatteringRadius" ) {

  GIVEN( "a parent node and a constant radius" ) {

    gnds::write::Options options;

    THEN( "a scatteringRadius node can be inserted" ) {

      pugi::xml_document parent;
      auto node = gnds::write::resonances::insertScatteringRadius( parent, options, 1., 1e-5, 2e+7, "eval" );
  ...
  GIVEN( "a parent node and a tabulated radius" ) {
    ...
    THEN( "a scatteringRadius node can be inserted" ) {
      ...
      pugi::xml_document parent;
      auto node = gnds::write::resonances::insertScatteringRadius( parent, options, radius, 1e-5, 2e+7, "eval" );
