...
SCENARIO( "createLegendreAngularDistributions" ) {

  GIVEN( "GNDS legendre angular data" ) {
    ...
    WHEN( "a single two body data node is given" ) {
      ...
      THEN( "it can be converted" ) {
        ...
        auto chunk1 = gnds::read::createLegendreAngularDistributions( xys2d, false );
        auto chunk2 = gnds::read::createLegendreAngularDistributions( xys2d, true );
...
void verifyChunk( const LegendreAngularDistributions& chunk ) {
    ...
    CHECK( 8 == chunk.distributions()[0].cdf().coefficients().size() );
    CHECK( 8 == chunk.distributions()[1].cdf().coefficients().size() );
    CHECK( 8 == chunk.distributions()[151].cdf().coefficients().size() );
    CHECK( 8 == chunk.distributions()[152].cdf().coefficients().size() );
