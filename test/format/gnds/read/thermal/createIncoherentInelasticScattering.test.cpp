  ...
  GIVEN( "GNDS incoherent elastic node from tsl data" ) {
    ...
    WHEN( "a single incoherent elastic node is given" ) {
      ...
      THEN( "it can be converted" ) {
        ...
        auto chunk = gnds::read::thermal::createIncoherentInelasticScattering( 1e-5, 10., 1.00866491574, incoherent );
...
void verifyChunk( const thermal::IncoherentInelasticScattering& chunk ) {
  ...
  CHECK_THAT( 6.33787160831, WithinRel( chunk.boundCrossSection() ) );
  CHECK_THAT( 91.2196203199 / 1.00866491574, WithinRel( chunk.atomicWeightRatio() ) );
