  ...
  GIVEN( "instances of ContinuousEnergyTable" ) {
    ...
    WHEN( "a photonuclear table is given" ) {
      ...
      THEN( "tabulated cross sections can be derived" ) {
        ...
        auto xs = format::ace::continuous::createTabulatedCrossSection( table, 1 );
        ...
        CHECK_THAT( 16.72987, WithinRel( xs.values().front() ) );
        CHECK_THAT( 2.710792e-5, WithinRel( xs.values().back() ) );
