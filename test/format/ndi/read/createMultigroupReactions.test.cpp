...
SCENARIO( "createMultigroupReactions" ) {
  ...
  GIVEN( "an NDI library" ) {

    WHEN( "a single NDI table is given" ) {
    ...
    id::ParticleID target( "H1" );
        ...
        auto reactions = ndi::read::createMultigroupReactions( projectile, target, table, boundaries );

        CHECK( 3 == reactions.size() );

        auto reaction = reactions[0];
        neutron::h1::verifyTotalReaction( reaction );

        reaction = reactions[1];
        neutron::h1::verifyElasticReaction( reaction );

        reaction = reactions[2];
        neutron::h1::verifyCaptureReaction( reaction );
      } // THEN
    } // WHEN
  } // GIVEN
} // SCENARIO
