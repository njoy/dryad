...
SCENARIO( "createMultigroupReaction" ) {
  ...
  GIVEN( "a GROUPR formatted GENDF material" ) {
    ...
    WHEN( "a primary MT number is given" ) {
      ...
      THEN( "a primary MultigroupReaction with products is created" ) {
        ...
        neutron::u235::verifyElasticReaction( elastic );
  ...
  GIVEN( "an ERRORR formatted GENDF material" ) {
    ...
    WHEN( "a lumped covariance MT number is given" ) {

      auto lumped = gendf::read::createMultigroupReaction( projectile, target, material, 851, boundaries, dilution );

      THEN( "a summation MultigroupReaction with partials is created" ) {

        neutron::u235::verifyLumped1Reaction( lumped );
      } // THEN
    } // WHEN
  } // GIVEN
} // SCENARIO
